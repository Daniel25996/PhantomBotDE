...
$(function () {
    ...
    async function showAddEditModal(isEdit, command, e, cooldownJson, perm, perms) {
        ...
        let modalId = (isEdit ? 'edit' : 'add') + '-command',
                modalTitle = (isEdit ? 'Bearbeiten' : 'Hinzufügen') + ' Befehl';
        helpers.getAdvanceModal(modalId, modalTitle, 'Speichern', $('<form/>', {
            'role': 'form'
        })
                // Append input box for the command name. This one is disabled.
                .append(helpers.getInputGroup('command-name', 'text', 'Befehl', '', '!' + command, 'Name des Befehls.' + (isEdit ? ' Dies kann nicht bearbeitet werden.' : ''), isEdit))
                // Append a text box for the command response.
                .append(helpers.getTextAreaGroup('command-response', 'text', 'Antwort', '', helpers.getDefaultIfNullOrUndefined(e.discordCommands, ''), 'Antwort des Befehls.'))
                ...
                .append($('<div/>', {
                    ...
                    'html': $('<form/>', {
                        'role': 'form'
                            ...
                            .append(await getChannelSelector('command-channel', 'text', 'Kanal', '#commands', helpers.getDefaultIfNullOrUndefined(e.discordChannelcom, ''),
                ...
                default:
                    ...
                    socket.updateDBValues('discord_custom_command_add', {
                        ...
                        socket.wsEvent('discord_custom_command_cooldown_ws', './discord/core/commandCoolDown.js', null,
                            ...
                            toastr.success('Befehl !' + commandName.val() + ' erfolgreich ' + (isEdit ? 'bearbeitet.' : 'hinzugefügt.');
        ...
        socket.getDBValue('discord_custom_command_module', 'modules', './discord/commands/customCommands.js', function (e) {
            ...
            socket.getDBTableValues('discord_commands_get_custom', 'discordCommands', function (results) {
                ...
                table.on('click', '.btn-danger', function () {
                    ...
                    helpers.getConfirmDeleteModal('custom_command_modal_remove', 'Bist du sicher, dass du den Befehl !' + command + ' entfernen willst?', true,
