...
$(run = function () {
    socket.getDBValues('raffle_module_status_toggle', {
        ...
        if (location.protocol.toLowerCase().startsWith('https') && !(location.port > 0 && location.port !== 443)) {
            ...
            $('#raffle-chat').html('Aufgrund von Änderungen durch Twitch kann das Chat-Panel nicht mehr angezeigt werden, es sei denn, du aktivierst SSL im PhantomBot-Panel und änderst den Baseport auf 443. Dies funktioniert möglicherweise nicht ohne Root-Privilegien.<br /><br />Alternativ können Sie sich mit der GitHub-Version des Panels bei <a href="https://phantombot.dev/">PhantomBot</a> anmelden, die dieses Problem umgeht.<br /><br />Hilfe beim Einrichten von SSL finden Sie in <a href="https://phantombot.dev/guides/#guide=content/integrations/twitchembeds">diesem Handbuch</a>.');
        ...
        switch (false) {
            ...
            default:
                socket.sendCommandSync('draw_raffle_cmd', 'raffle draw ' + drawAmount.val() + ' ' + prize.val(), function () {
                    // Alert the user.
                    toastr.success('Erfolgreich ' + drawAmount.val() + ' Gewinner' + (drawAmount.val() === 1 ? '' : 's') + ' gezogen!');
                    // Reload
                    helpers.temp.loadRaffleList();
                });
        }
