...
if [[ ! -O "PhantomBot.jar" ]]; then
    echo "Das Verzeichnis wird nicht vom Dienstbenutzer geändert"
    echo "Bitte führe den folgenden Befehl aus, um dies zu beheben:"
    echo "   sudo chown ${EUID} ${pwd}"

    exit 1
fi

${JAVA} --add-exports java.base/sun.security.x509=ALL-UNNAMED --add-opens java.base/java.lang=ALL-UNNAMED ${tmp} -Duser.language=en -Djava.security.policy=config/security -Xms1m -Dfile.encoding=UTF-8 -jar PhantomBotDE.jar ${1}
